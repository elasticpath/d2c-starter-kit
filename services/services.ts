...
export async function getNodes(id: string): Promise<moltin.Node[]> {
  const moltin = MoltinGateway({
    host: config.endpointURL,
    client_id: config.clientId,
    client_secret: config.clientSecret,
  });
  const result = await moltin.Hierarchies.Children(id);
...
export async function getNode(
  ...
  const result = await moltin.Hierarchies.Nodes.Get({
    hierarchyId,
    nodeId,
  });
  const nodes = result.data;
...
export async function getNodesProducts(
  ...
  const endpoint = await moltin.Hierarchies.Relationships;
  const productsList = await endpoint.Products({ hierarchyId, nodeId });
  return productsList;
}
